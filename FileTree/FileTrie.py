...
class FileTrie:
    ...
    def remove(self, filename):
        filename = filename.replace(".html","")
        if not self.lookup(filename): return
        curr = self.root
...
if __name__ == '__main__':
    ...
    print(f.lookup("User2_"+str(datetime.datetime.now()).replace(" ","-")))
    print(f.lookup("Usasfer1_"+str(datetime.datetime.now()).replace(" ","-")))
    print(f.remove("User1_"+str(datetime.datetime.now()).replace(" ","-")))
